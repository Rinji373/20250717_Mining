...
for(i in 1:nrow(polygon)){
  ...
  for(month in 1:12){
    ...
    maskWithSCL <- function(img){
      scl <- img$select("SCL")
      mask <- scl$neq(1)$And(scl$neq(11))$And(scl$neq(3))$And(scl$neq(8))$And(scl$neq(9))$And(scl$neq(10))   # データなし、雪、雲影、巻雲、雲をマスク
    ...
    validPixelCount <- function(img){
      validPixels <- img$reduceRegion(
        ...
        geometry = region,
        scale = 100,
        bestEffort = TRUE,  # 自動的にスケールを調整
      )$get("B1")  # 任意のバンドで有効画素数を取得
      return(img$set("valid_pixel_count", validPixels))
    }
    
    LC08_with_valid_count <- LC08_masked$map(validPixelCount)
    best_masked_image <- LC08_with_valid_count$sort("valid_pixel_count", FALSE)$first()  # 有効画素数が最大の画像を取得
    ...
    filename_best_unmasked <- paste0("Sentinel2_Best_Image_polygon",polygon1$group_id,sprintf("-%02d", month))
