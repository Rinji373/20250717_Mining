...
# 鉱山ポリゴンの読み込み（SHAPE形式）
polygon <- st_read(dsn="C:/Users/casakuma-lab-04-std/Downloads/merged_polygon_test1.shp")
...
for(i in 1:nrow(polygon)){
  ...
  #### 雲マスク前の画像から中央値合成画像
  composite_unmasked <- LC08$select(c('B1','B2','B3','B4','B5','B6','B7','B8','B8A','B9','B11','B12'))$median()
  ...
  # 各画像の年間最小値合成画像を作成
  composite_1 <- LC08_masked_1$select(c('B1','B2','B3','B4','B5','B6','B7','B8','B8A','B9','B11','B12'))$median()
  composite_2 <- LC08_masked_2$select(c('B1','B2','B3','B4','B5','B6','B7','B8','B8A','B9','B11','B12'))$median()
  composite_3 <- LC08_masked_3$select(c('B1','B2','B3','B4','B5','B6','B7','B8','B8A','B9','B11','B12'))$median()
  composite_4 <- LC08_masked_4$select(c('B1','B2','B3','B4','B5','B6','B7','B8','B8A','B9','B11','B12'))$median()
  composite_5 <- LC08_masked_5$select(c('B1','B2','B3','B4','B5','B6','B7','B8','B8A','B9','B11','B12'))$median()
  ...
  createMaskedSCLImage <- function(img, mask_conditions, new_value){
    ...
    return(scl$updateMask(mask)$multiply(0)$add(new_value))  # 値を変更
  }
  ...
  filename_combined <- paste("Sentinel-2_Yearly_MedianComposite_polygon", polygon1$group_id,
                           gsub("-", "", start_date),
                           gsub("-", "", end_date), sep = "_")
  task_combined <- ee_image_to_drive(
    ...
    timePrefix = FALSE,
    folder = "Sentinel-2_Yearly_MedianComposite",
    region = region,
    scale = 10
  )
  task_combined$start()
